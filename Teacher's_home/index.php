...
if (isset($_GET['type'])==false){
  ...
  $temp = $_GET['type'];
  $_GET['type'] ="and request_type='".$_GET['type']."'";
...
if (isset($_GET['sortby'])==false){
  ...
  $temp = $_GET['sortby'];
  if( $temp=='reqiest_from'){
    $temp= 'Name';}
  $_GET['sortby'] =",".$_GET['sortby']." DESC" ;
...
<?php include '../navbar2.php' ?>
          ...
          ?>
                ...
                if($_SESSION['status'] !="all"){
                  ...
                  $sql2 =  "SELECT id, is_read, request_to, request_type, detail FROM request WHERE request_to ='{$_SESSION['name']}' and status='{$_SESSION['status']}' {$_GET['type']}";
                  $sql = "SELECT id,is_read, request_to,reqiest_from, request_type, detail FROM request WHERE request_to ='{$_SESSION['name']}' and status='{$_SESSION['status']}'{$_GET['type']} ORDER BY time DESC {$_GET['sortby']} LIMIT 10 OFFSET $offset ";
                  }
                  else{
                    $sql2 =  "SELECT id,is_read, request_to, request_type, detail FROM request WHERE request_to ='{$_SESSION['name']}' {$_GET['type']}";
                   
                  $sql = "SELECT id,is_read, request_to,reqiest_from, request_type, detail FROM request WHERE request_to ='{$_SESSION['name']}' {$_GET['type']} ORDER BY  time DESC {$_GET['sortby']} LIMIT 10 OFFSET $offset ";
                  ...
                  while($row = $result->fetch_assoc()) {
                   ?>
                    ...
                    <div class="d-flex w-100 justify-content-between">
                      ...
                      <h5 class="mb-1"><span class="pr-5"><?php echo $row['reqiest_from'] ?> </span>  - <span class="pl-5"> <?php echo $row['request_type'] ?></span></h5>
                      <h5 > <i class="fa 
